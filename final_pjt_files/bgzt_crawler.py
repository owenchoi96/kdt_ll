...
def prd_crawler(prd_id): 
    ...
    if res.status_code == 200: # inspired by Nara's code
        data = res.json()
        try:
            ...
            # -- 추가된 정보 --
            cat_name = data['data']['product']['category']['name']
...
def product_mp(prd_list:list):
    ...
    pool = multiprocessing.Pool(5) # cpu 개수 조절하기 => 4로 줄임
...
if __name__ == '__main__':
    ...
    for idx in range(9): # 분할해서 데이터 프레임으로 만들기 
        ...
        result.to_csv(f'bungae_df_{idx}_fashion.csv', index=False)
